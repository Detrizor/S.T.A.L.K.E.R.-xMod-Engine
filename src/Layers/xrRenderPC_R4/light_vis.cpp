...
void	light::vis_prepare			()
{
	...
	//Msg	("dist:%f, sa:%f",Device.camera.position.distance_to(spatial.center),safe_area);
	bool	skiptest	= false;
	...
	if (skiptest || Device.camera.position.distance_to(spatial.sphere.P)<=(spatial.sphere.R*1.01f+safe_area))	{	// small error
		vis.visible		=	true;
		vis.pending		=	false;
