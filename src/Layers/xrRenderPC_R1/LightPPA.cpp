...
void CLightR_Manager::render_point	()
{
	...
	Device.camera.project._43			-= 0.001f; 
	...
	for (xr_vector<light*>::iterator it=selected_point.begin(); it!=selected_point.end(); it++)
	{
		...
		float	alpha		= Device.camera.position.distance_to(PPL.position)/MAX_DISTANCE;
		if (alpha>=1)																continue;
		...
		RCache.set_c		(hPPAcolor,factor.r,factor.g,factor.b,1);
		{
			...
			Fvector	cam		= Device.camera.position;
...
{
	// for each light
	Fvector		lc_COP		= Device.camera.position	;
	...
	for (xr_vector<light*>::iterator it=selected_point.begin(); it!=selected_point.end(); it++)
	{
		...
		bHUD				= F.testSphere_dirty	(Device.camera.position,2.f);
