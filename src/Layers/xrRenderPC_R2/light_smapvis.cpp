...
void	smapvis::invalidate	()
{
	...
	testQ_V		=	0;
	frame_sleep	=	::Render->dwFrame() + ps_r__LightSleepFrames;
	invisible.clear	();
}
void	smapvis::begin		()
{
...
void	smapvis::end		()
{
	...
	case state_working:
		...
		if (testQ_V)
		{
			...
			testQ_frame								= ::Render->dwFrame() + 1;	// get result on next frame
		}
		break;
	case state_usingTC:
		// nothing to do
...
{
	// the tough part
	if (testQ_frame != ::Render->dwFrame())			return;
...
{
	if (testQ_frame==(::Render->dwFrame()+1))		testQ_frame--;
	flushoccq		();
}
