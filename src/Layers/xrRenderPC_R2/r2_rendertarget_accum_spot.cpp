...
void CRenderTarget::accum_spot	(light* L)
{
	...
	Fmatrix			m_Shadow,m_Lmap;
	{
		...
		// compute xforms
		Fmatrix			xf_world;		xf_world.invert	(Device.camera.view);
	...
	Device.camera.view.transform_dir	(L_dir,L->direction);
	L_dir.normalize				();
...
void CRenderTarget::accum_volumetric(light* L)
{
	...
	for (u32 i=0; i<8; i++)		{
		...
		Device.camera.full_transform.transform	(mView);
		bbp.modify	(pt);
	}
	*/
	...
	//Fvector	rr = Fvector().set(scaledRadius,scaledRadius,scaledRadius);
	//Fvector pt = L->spatial.sphere.P;
	Device.camera.view.transform(pt);
	aabb.setb( pt, rr);
/*	
	//	Calculate presise AABB assuming we are drawing for the spot light
	{
		aabb.invalidate();
		Fmatrix	transform;
		transform.mul( Device.camera.view, L->m_xform);		 
	...
	// Draw volume with projective texgen
	{
		...
		*/
		...
		//	Set up user clip planes
		{
			...
			PlaneTransform.transpose(Device.camera.full_transform_inv);
