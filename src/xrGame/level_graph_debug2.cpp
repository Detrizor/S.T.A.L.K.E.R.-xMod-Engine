...
//	F->Out				(0.f,0.55f,"%3d,%4d,%3d -> %d",	iFloor(x),iFloor(Local.y()),iFloor(z),u32(ID));
	...
	max_position = min_position = Device.camera.position;
	...
	for ( ; I != E; ++I)
	{
		...
		if (Device.camera.view_base.testSphere_dirty(PC,sr)) {
			...
			if (bHL) {
				...
				Fvector4	S;
				T.set		(PC); T.y+=0.3f;
				Device.camera.full_transform.transform	(S,T);
...
void CLevelGraph::draw_covers	()
{
	...
	ai().cover_manager().covers().nearest(Device.camera.position,5.f,nearest);
