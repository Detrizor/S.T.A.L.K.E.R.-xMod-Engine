...
IC void FillSprite_Line	(const Fvector& pos, const Fvector& dir, const float width, const float length, u32 color)
{
    const Fvector& T	= dir;
    Fvector				R;      
	R.crossproduct		(T,Device.camera.direction).normalize_safe();
...
void  CTracer::Render	(const Fvector& pos, 
						 ...
						 bool bActor)
{
	if (Device.camera.view_base.testSphere_dirty((Fvector&)center,length*.5f) )
	{
...
//			float f_distance	= Device.camera.position.distance_to(pos);
