...
void	CCar::Hit							(SHit* pHDS)
{
	...
	float hitScale = 1.f, pierceScale = -1.f;
	if (HDS.hit_type != ALife::eHitTypeStrike) CDamageManager::HitScale(HDS.bone(), hitScale, pierceScale);
	HDS.main_damage *= GetHitImmunity(HDS.hit_type) * hitScale;
...
void CCar::RefreshDriveWheelSigns()
{
	if (!m_pPhysicsShell || m_driving_wheels.empty())
		return;

	auto kinematics{ smart_cast<IKinematics*>(Visual()) };
	if (!kinematics)
		return;

	auto rootBoneIt{ bone_map.find(kinematics->LL_GetBoneRoot()) };
	if (rootBoneIt == bone_map.end() || !rootBoneIt->second.element)
		return;

	m_root_transform.set(rootBoneIt->second.element->mXFORM);
	for (auto& wheel : m_driving_wheels)
		wheel.UpdateDriveSign();
}
