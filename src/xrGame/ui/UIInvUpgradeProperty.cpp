...
void UIInvUpgPropertiesWnd::initFromXml(CUIXml& xmlDoc)
{
	XML_NODE* stored_root = xmlDoc.GetLocalRoot();
	XML_NODE* node = xmlDoc.NavigateToNode("upgrade_info", 0);
	xmlDoc.SetLocalRoot(node);

	CUIXmlInit::InitWindow(xmlDoc, "properties", 0, this);

	m_Upgr_line = xr_new<CUIStatic>();
	AttachChild(m_Upgr_line);
	m_Upgr_line->SetAutoDelete(true);
	CUIXmlInit::InitStatic(xmlDoc, "properties:upgr_line", 0, m_Upgr_line);
	...
	for (; ib != ie; ++ib)
	{
		...
		ui_property->init_from_xml(xmlDoc);
	...
	xmlDoc.SetLocalRoot(stored_root);
}

void UIInvUpgPropertiesWnd::initFromXml(LPCSTR strXmlName)
{
	CUIXml uiXml{};
...
void UIInvUpgPropertiesWnd::setInfo(PIItem item)
{
