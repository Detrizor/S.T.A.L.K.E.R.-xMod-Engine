...
void manager::load_weathers()
{
	...
	for (auto& pair : WeatherCycles)
	{
		R_ASSERT3(pair.second.size() > 1, "Environment in weather must >=2", pair.first.c_str());
		pair.second.sort(sort_env_etl_pred);
	}
...
manager::shader_ids_type const& manager::shader_ids() const
{
	...
	m_shader_ids.sort(logical_string_predicate());
...
manager::particle_ids_type const& manager::particle_ids() const
{
	...
	m_particle_ids.sort(logical_string_predicate());
	return (m_particle_ids);
}
...
manager::light_animator_ids_type const& manager::light_animator_ids() const
{
	...
	m_light_animator_ids.sort(logical_string_predicate());
