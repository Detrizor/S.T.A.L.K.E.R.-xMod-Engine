...
void CLensFlare::OnFrame(shared_str id)
{
	...
	matEffCamPos.i.set(Device.camera.right);
	matEffCamPos.j.set(Device.camera.top);
	matEffCamPos.k.set(Device.camera.direction);
	vecPos.set(Device.camera.position);
	...
	R_ASSERT(_valid(vSunDir));
	STranspParam TP(&m_ray_cache[0], Device.camera.position, vSunDir, 1000.f, EPS_L);
	...
	/*
	CObject* o_main = g_pGameLevel->CurrentViewEntity();
	STranspParam TP (&m_ray_cache,Device.camera.position,vSunDir,1000.f,EPS_L);
	collide::ray_defs RD (TP.P,TP.D,TP.f,CDB::OPT_CULL,collide::rqtBoth);
	...
	*/
	/*
	 CObject* o_main = g_pGameLevel->CurrentViewEntity();
	 STranspParam TP (this,Device.camera.position,vSunDir,1000.f,EPS_L);
	 collide::ray_defs RD (TP.P,TP.D,TP.f,CDB::OPT_CULL,collide::rqtBoth);
	 if (m_ray_cache.result&&m_ray_cache.similar(TP.P,TP.D,TP.f)){
	 // similar with previous query == 0
	 TP.vis = 0.f;
	...
	{
		Fvector scr_pos;
		Device.camera.full_transform.transform(scr_pos, vecLight);
		float kx = 1, ky = 1;
